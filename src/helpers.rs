        ...
        match ctx {
            Context::Application(_) => {
                if let Err(e) = ctx.say(format!("❌ {error}")).await {
                    warn!("Failed to send failure acknowledgment slash command response: {}", e);
                }
            }
            Context::Prefix(prefix_context) => {
                if let Err(e) = prefix_context.msg.react(ctx, serenity::ReactionType::from('❌')).await {
...
pub async fn acknowledge_success(ctx: Context<'_>, emoji_name: &str, fallback: char) -> Result<(), Error> {
    let emoji = find_custom_emoji(ctx, emoji_name);
    match ctx {
        Context::Prefix(prefix_context) => {
            let reaction = emoji.map_or_else(|| serenity::ReactionType::from(fallback), serenity::ReactionType::from);
...
pub async fn trim_text(
    ...
    let needs_truncating =
        text_body.len() + text_end.len() > MAX_OUTPUT_LENGTH || text_body.lines().count() > MAX_OUTPUT_LINES;
    ...
    if needs_truncating {
        ...
        let text_body: String =
            text_body.chars().take(MAX_OUTPUT_LENGTH - truncation_msg.len() - text_end.len()).collect();

        // truncate for lines
        let text_body = text_body.lines().take(MAX_OUTPUT_LINES).collect::<Vec<_>>().join("\n");
