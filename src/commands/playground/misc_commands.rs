...
use super::{
    api::{apply_online_rustfmt, ClippyRequest, CrateType, MacroExpansionRequest, MiriRequest, PlayResult},
    util::{
...
pub async fn miri(ctx: Context<'_>, flags: poise::KeyValueArgs, code: poise::CodeBlock) -> Result<(), Error> {
    ctx.say(stub_message(ctx)).await?;
    let code = &maybe_wrapped(&code.code, ResultHandling::Discard, ctx.prefix().contains("Sweat"), false);
    ...
    let mut result: PlayResult = ctx
        ...
        .json(&MiriRequest { code, edition: flags.edition, aliasing_model: flags.aliasing_model })
        .send()
        .await?
        .json()
        .await?;

    result.stderr = extract_relevant_lines(&result.stderr, &["Running `/playground"], &["error: aborting"]).to_owned();
...
#[poise::command(prefix_command, track_edits, help_text_fn = "expand_help", category = "Playground")]
pub async fn expand(ctx: Context<'_>, flags: poise::KeyValueArgs, code: poise::CodeBlock) -> Result<(), Error> {
    ctx.say(stub_message(ctx)).await?;
    ...
    result.stderr =
        extract_relevant_lines(&result.stderr, &["Finished ", "Compiling playground"], &["error: aborting"]).to_owned();

    if result.success {
        match apply_online_rustfmt(ctx, &result.stdout, flags.edition).await {
            Ok(PlayResult { success: true, stdout, .. }) => result.stdout = stdout,
            Ok(PlayResult { success: false, stderr, .. }) => warn!(
...
#[poise::command(prefix_command, track_edits, help_text_fn = "clippy_help", category = "Playground")]
pub async fn clippy(ctx: Context<'_>, flags: poise::KeyValueArgs, code: poise::CodeBlock) -> Result<(), Error> {
    ctx.say(stub_message(ctx)).await?;
...
pub async fn fmt(ctx: Context<'_>, flags: poise::KeyValueArgs, code: poise::CodeBlock) -> Result<(), Error> {
    ctx.say(stub_message(ctx)).await?;
