...
impl<'de> Deserialize<'de> for PlayResult {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(match RawPlayResponse::deserialize(deserializer)? {
            RawPlayResponse::Ok { success, stdout, stderr } => PlayResult { success, stdout, stderr },
            RawPlayResponse::Err { error } => PlayResult { success: false, stdout: String::new(), stderr: error },
        })
    }
}
...
pub async fn apply_online_rustfmt(ctx: Context<'_>, code: &str, edition: Edition) -> Result<PlayResult, Error> {
    let result = ctx
        .data()
        .http
