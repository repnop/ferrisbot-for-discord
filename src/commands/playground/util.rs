...
pub fn parse_flags(mut args: poise::KeyValueArgs) -> (api::CommandFlags, String) {
    ...
    macro_rules! pop_flag {
        ($flag_name:literal, $flag_field:expr) => {
            if let Some(flag) = args.0.remove($flag_name) {
                match flag.parse() {
                    Ok(x) => $flag_field = x,
                    Err(e) => writeln!(errors, "{e}").expect("Writing to a String should never fail"),
                }
            }
        };
    }
    ...
    for (remaining_flag, _) in args.0 {
        writeln!(errors, "unknown flag `{remaining_flag}`").expect("Writing to a String should never fail");
    }
...
pub fn maybe_wrapped(code: &str, result_handling: ResultHandling, unsf: bool, pretty: bool) -> Cow<'_, str> {
...
pub async fn send_reply(
    ...
    let timeout = result.contains("Killed") && result.contains("timeout") && result.contains("--signal=KILL");
    ...
    let text = crate::helpers::trim_text(&format!("{flag_parse_errors}```rust\n{result}"), &text_end, async {
        format!(
    ...
    let response = ctx
        .send({
            ...
            if timeout {
                b = b.components(vec![serenity::CreateActionRow::Buttons(vec![serenity::CreateButton::new(
                    &custom_id,
                )
                .label("Retry")
...
pub fn strip_fn_main_boilerplate_from_formatted(text: &str) -> String {
    ...
    let text = match (text.find(prefix), text.rfind(postfix)) {
        (Some(prefix_pos), Some(postfix_pos)) => text.get((prefix_pos + prefix.len())..postfix_pos).unwrap_or(text),
        _ => text,
    };
    let text = text.trim();
...
pub fn stub_message(ctx: Context<'_>) -> String {
    ...
    if let Context::Prefix(ctx) = ctx {
        if let Some(edit_tracker) = &ctx.framework.options().prefix_options.edit_tracker {
            if let Some(existing_response) = edit_tracker.read().unwrap().find_bot_response(ctx.msg.id) {
