...
pub async fn thread_pin(ctx: Context<'_>, message: serenity::Message) -> Result<()> {
    let reply = match can_pin_in_thread(ctx).await {
        ...
        Err(ThreadPinError::NotThreadOwner) => "You did not create this thread, so cannot pin messages to it.",
        Err(ThreadPinError::ThreadLocked) => "This thread has been locked, so this cannot be performed.",
    };
