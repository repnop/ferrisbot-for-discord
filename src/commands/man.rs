...
#[poise::command(prefix_command, slash_command, broadcast_typing, category = "Utilities")]
pub async fn man(
    ctx: Context<'_>,
    ...
    if let Ok(response) = ctx.data().http.get(&url).header(header::USER_AGENT, USER_AGENT).send().await {
    ...
    ctx.send(
        poise::CreateReply::default().embed(
            serenity::CreateEmbed::new()
                ...
                .footer(serenity::CreateEmbedFooter::new("Powered by manpages.debian.org"))
