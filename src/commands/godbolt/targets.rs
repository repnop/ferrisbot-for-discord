...
async fn update_godbolt_metadata(data: &Data) -> Result<(), Error> {
    ...
    let needs_update = if let Some(last_update_time) = last_update_time {
        ...
        let time_since_update = std::time::Instant::now().saturating_duration_since(last_update_time);
        let needs_update = time_since_update >= update_period;
        if needs_update {
            info!("godbolt metadata was last updated {:#?} ago, updating it", time_since_update,);
        }
    ...
    if needs_update {
        let request =
            data.http.get("https://godbolt.org/api/compilers/rust").header(reqwest::header::ACCEPT, "application/json");
        ...
        let request =
            data.http.get("https://godbolt.org/api/libraries/rust").header(reqwest::header::ACCEPT, "application/json");
        let libraries: Vec<GodboltLibrary> = request.send().await?.json().await?;

        info!("updating godbolt metadata: {} targets, {} libraries", targets.len(), libraries.len());
        *data.godbolt_metadata.lock().unwrap() =
            GodboltMetadata { targets, libraries, last_update_time: Some(std::time::Instant::now()) };
    }

    Ok(())
}

pub async fn fetch_godbolt_metadata(data: &Data) -> impl std::ops::Deref<Target = GodboltMetadata> + '_ {
...
pub(crate) async fn rustc_id_and_flags(data: &Data, params: &poise::KeyValueArgs) -> Result<(String, String), Error> {
    let rustc = params.get("rustc").unwrap_or("nightly");
    let target =
        fetch_godbolt_metadata(data).await.targets.iter().find(|target| target.semver == rustc.trim()).cloned().ok_or(
            anyhow!(
    ...
    let flags = itertools::Itertools::intersperse(
        params
            .0
            .iter()
            ...
            .chain(once(format!("--edition={edition}"))),
        " ".to_string(),
    )
    ...
    fn from(semver: &'a str) -> Self {
        match semver {
            ...
            semver => {
                ...
                let version_triple =
                    version_triple.next().zip(version_triple.next()).zip(version_triple.next()).and_then(
                        |((major, minor), patch)| Some((major.parse().ok()?, minor.parse().ok()?, patch.parse().ok()?)),
                    );
...
pub async fn targets(ctx: Context<'_>) -> Result<(), Error> {
    ...
    targets.sort_unstable_by(|lhs, rhs| SemverRanking::from(&*lhs.semver).cmp(&SemverRanking::from(&*rhs.semver)));

    ctx.send(
        poise::CreateReply::default().embed(
            serenity::CreateEmbed::default().title("Godbolt Targets").fields(
                targets.into_iter().map(|target| {
                    (target.semver, format!("{} (runs on {})", target.name, target.instruction_set), true)
                }),
            ),
        ),
    )
