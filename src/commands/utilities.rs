...
/// Evaluates Go code
#[poise::command(prefix_command, slash_command, category = "Utilities", discard_spare_arguments)]
...
pub async fn source(ctx: Context<'_>) -> Result<(), Error> {
    ctx.say("https://github.com/rust-community-discord/ferrisbot-for-discord").await?;
    Ok(())
}
...
You can edit your message to the bot and the bot will edit its response.";
    ...
    poise::builtins::help(
        ctx,
        command.as_deref(),
        poise::builtins::HelpConfiguration { extra_text_at_bottom, ephemeral: true, ..Default::default() },
    )
    .await?;
    Ok(())
}
...
pub async fn uptime(ctx: Context<'_>) -> Result<(), Error> {
    ...
    ctx.say(format!("Uptime: {days}d {hours}h {minutes}m {seconds}s")).await?;
...
#[poise::command(prefix_command, slash_command, category = "Utilities", track_edits, hide_in_help)]
pub async fn conradluget(
    ...
    static FONT: LazyLock<ab_glyph::FontRef<'_>> = LazyLock::new(|| {
        ab_glyph::FontRef::try_from_slice(include_bytes!("../../assets/OpenSans.ttf")).expect("failed to load font")
    });

    let text = format!("Get {text}");
    let image =
        imageproc::drawing::draw_text(&*BASE_IMAGE, image::Rgba([201, 209, 217, 255]), 57, 286, 65.0, &*FONT, &text);
...
#[poise::command(prefix_command, slash_command, category = "Utilities", on_error = "crate::helpers::acknowledge_fail")]
pub async fn cleanup(
    ...
    let messages_to_delete = ctx
        ...
        .into_iter()
        .filter(|msg| {
            (msg.author.id == ctx.data().application_id) && (*ctx.created_at() - *msg.timestamp).num_hours() < 24
        })
        .take(num_messages);
...
/// or in minutes.
#[poise::command(slash_command, category = "Utilities", on_error = "crate::helpers::acknowledge_fail")]
pub async fn selftimeout(
    ...
    let mut member = ctx.author_member().await.ok_or(anyhow!("failed to fetch member"))?.into_owned();

    member.disable_communication_until_datetime(&ctx, then).await?;
