...
async fn compile_rust_source(http: &reqwest::Client, request: &GodboltRequest<'_>) -> Result<Compilation, Error> {
    ...
    let http_request = http
        .post(format!("https://godbolt.org/api/compiler/{}/compile", request.rustc))
    ...
    Ok(Compilation {
        output: if request.run_llvm_mca {
            ...
            // Strip junk
            text[..text.find("Instruction Info").unwrap_or(text.len())].trim().to_string()
        } else {
...
async fn save_to_shortlink(http: &reqwest::Client, req: &GodboltRequest<'_>) -> String {
    ...
    let request = http.post("https://godbolt.org/api/shortener").json(&serde_json::json! { {
        "sessions": [{
            "language": "rust",
    ...
    let url = async move { Ok::<_, crate::Error>(request.send().await?.json::<GodboltShortenerResponse>().await?.url) };
...
async fn respond_codeblock(
    ...
    ctx.say(
        crate::helpers::trim_text(&format!("```{codeblock_lang}\n{text}"), &format!("\n```{note}"), async {
            format!("Output too large. Godbolt link: <{}>", save_to_shortlink(&ctx.data().http, godbolt_request).await,)
        })
        .await,
    )
    .await?;
...
pub async fn godbolt(ctx: Context<'_>, #[rest] arguments: String) -> Result<(), Error> {
    ...
    let godbolt_request = GodboltRequest { source_code: &code, rustc: &rustc, flags: &flags, run_llvm_mca: false };
...
pub async fn mca(ctx: Context<'_>, #[rest] arguments: String) -> Result<(), Error> {
    ...
    let no_mangle_added = add_no_mangle(&mut code);
    let (rustc, flags) = rustc_id_and_flags(ctx.data(), &params).await?;
    let godbolt_request = GodboltRequest { source_code: &code, rustc: &rustc, flags: &flags, run_llvm_mca: true };
