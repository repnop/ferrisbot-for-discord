...
async fn get_crate(http: &reqwest::Client, query: &str) -> Result<Crate> {
    ...
    let crate_ = crate_list.crates.into_iter().next().ok_or_else(|| anyhow!("Crate `{}` not found", query))?;

    if crate_.exact_match {
        Ok(crate_)
    } else {
        bail!("Crate `{}` not found. Did you mean `{}`?", query, crate_.name)
    }
}
...
/// ?crate crate_name
/// ```
#[poise::command(prefix_command, slash_command, rename = "crate", broadcast_typing, category = "Crates")]
pub async fn crate_(
    ctx: Context<'_>,
    ...
    ctx.send(
        poise::CreateReply::default().embed(
            serenity::CreateEmbed::new()
                ...
                .description(crate_.description.as_deref().unwrap_or("_<no description available>_"))
                .field(
                    "Version",
                    crate_.max_stable_version.or(crate_.max_version).unwrap_or_else(|| "<unknown version>".into()),
                    true,
                )
                .field("Downloads", format_number(crate_.downloads), true)
                .timestamp(crate_.updated_at.parse::<serenity::Timestamp>().unwrap_or(serenity::Timestamp::now()))
...
/// ```
#[poise::command(prefix_command, aliases("docs"), broadcast_typing, track_edits, slash_command, category = "Crates")]
pub async fn doc(
    ctx: Context<'_>,
    #[description = "Path of the crate and item to lookup"] query: String,
) -> Result<()> {
    ctx.say(path_to_doc_url(&query, &ctx.data().http).await?).await?;
...
async fn path_to_doc_url(query: &str, client: &impl DocsClient) -> Result<String> {
    ...
    if path.ident.is_none() {
        ...
        match is_in_std(path.crate_) {
            IsInStd::Primitive => {
                path = QualifiedPath { kind: Some("primitive"), crate_: "std", ident: Some(path.crate_), mods: "" };
            }
            IsInStd::PrimitiveNightly => {
                path = QualifiedPath { kind: Some("primitive"), crate_: "nightly", ident: Some(path.crate_), mods: "" };
            }
            IsInStd::Keyword(ident) => {
                path = QualifiedPath { kind: Some("keyword"), crate_: "std", ident: Some(ident), mods: "" };
            }
            IsInStd::PossibleType => {
                path = QualifiedPath { kind: path.kind, crate_: "std", ident: Some(path.crate_), mods: "" };
            }
            IsInStd::False => {}
        }
    }

    let (is_rustc_crate, mut doc_url, root_len) = if let Some(prefix) = rustc_crate_link(path.crate_) {
...
fn split_qualified_path(input: &str) -> QualifiedPath<'_> {
    ...
    let Some((crate_, rest)) = path.split_once("::") else {
        return QualifiedPath { kind, crate_: path, ident: None, mods: "" };
    };
    match rest.rsplit_once("::") {
        Some((mods, ident)) => QualifiedPath { kind, crate_, ident: Some(ident), mods },
        None => QualifiedPath { kind, crate_, ident: Some(rest), mods: "" },
    }
}
...
const UPPER_CAMEL_CASE_KINDS: &[&str] = &["struct", "enum", "union", "trait", "traitalias", "type", "derive"];
const SCREAMING_SNAKE_CASE_KINDS: &[&str] = &["constant", "static"];
const RUSTC_CRATE_ONLY_KINDS: &[&str] = &["keyword", "primitive"];

async fn guess_kind(client: &impl DocsClient, prefix: &str, is_rustc_crate: bool, ident: &str) -> Option<&'static str> {
    let mut attempt_order: Vec<&[&'static str]> = if ident.chars().next().is_some_and(char::is_lowercase) {
        vec![SNAKE_CASE_KINDS, UPPER_CAMEL_CASE_KINDS, SCREAMING_SNAKE_CASE_KINDS]
    } else if ident.chars().all(char::is_uppercase) {
        vec![SCREAMING_SNAKE_CASE_KINDS, UPPER_CAMEL_CASE_KINDS, SNAKE_CASE_KINDS]
    } else {
        vec![UPPER_CAMEL_CASE_KINDS, SCREAMING_SNAKE_CASE_KINDS, SNAKE_CASE_KINDS]
    };
    if is_rustc_crate {
...
impl DocsClient for reqwest::Client {
    async fn get_crate_docs(&self, crate_name: &str) -> Result<String> {
        get_crate(self, crate_name).await.map(|crate_| get_documentation(&crate_))
    }
